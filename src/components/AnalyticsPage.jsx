...
import { TicketMetricsAPI } from '../api/ticketsAPI';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
const AnalyticsPage = () => {
  ...
  const [ticketMetrics, setTicketMetrics] = React.useState(null);

  const TicketMetricsAPICall = async () => {
    const response = await TicketMetricsAPI();
    if (response.status) {
      setTicketMetrics(response.data);
    } else {
      console.log('Failed to fetch ticket metrics:', response.error);
    }

  }

  React.useEffect(() => {
    TicketMetricsAPICall()
  }, [])
        ...
        <>
          ...
          <Paper sx={{ mt: 3, mb: 3, p: 3, backgroundColor: '#242424', border: '1px solid #333333', width: '100%' }}>
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2 }}><KPICard metric={{ ...kpiMetrics.ticketsResolved, value: ticketMetrics?.totalTickets || 0 }} icon={CheckCircleIcon} /></Grid>
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2 }}><KPICard metric={{ ...kpiMetrics.slaCompliance, value: ticketMetrics?.totalSessions || 0 }} icon={ScheduleIcon} /></Grid>
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2 }}><KPICard metric={{ ...kpiMetrics.avgResolutionTime, value: ticketMetrics?.guardrailActivations || 0 }} icon={TimerIcon} /></Grid>
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2 }}><KPICard metric={{ ...kpiMetrics.aiAccuracy, value: ticketMetrics?.deflectionRate * 100 || 0 }} icon={PsychologyIcon} /></Grid>
              <Grid size={{ xs: 12, sm: 6, md: 4, lg: 2 }}><KPICard metric={{ ...kpiMetrics.tier0Resolution, value: ticketMetrics?.ticketsByTier?.TIER_2 || 0 }} icon={SelfImprovementIcon} /></Grid>
